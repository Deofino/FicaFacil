...
export default function Header () {
    ...
    const [ isDark, setDark ] = React.useState(storage === 'true' ? true : false || false);
    ...
    React.useEffect(() => {
        isDark ? document.querySelector('html').classList.add('dark')
    ...
    return (
        <header className={ "l-header " + isOpen }>
            ...
            <FormControlLabel
                control={
                    <Tooltip title='Habilitar/desabilitar modo escuro' arrow placement='top' >
                        <Switch inputProps={ { 'aria-label': "DarkMode" } } checked={ isDark } className='c-switch'
                            onChange={ (e) => dark(e) } />
                    </Tooltip>
                }
                label='Modo escuro'
