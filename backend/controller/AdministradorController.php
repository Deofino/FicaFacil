    ...
    public function create() // parametro do file_get_contents
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($data->administrador)) {
                $model->setNome(trim($data->administrador));
                $model->setEmail(trim($data->administrador));
                $model->setSenha(trim($data->administrador));// insere aqui pra passar pelas verificacoes de dados
                echo $model->post();
